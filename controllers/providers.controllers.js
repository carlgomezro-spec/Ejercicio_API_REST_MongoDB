...
const getAllproviders = async (req, res) => {
    try {
        const providers = await providersService.getProvidersService();
...
const postProvider = async(req,res) => {
    try{
        const nuevoProvider = await providersService.createProviderService(req.body) //nuevo Provider-> espera la funcion de crear provider de services-> indica que se pasa el nuevo modelo por el body (req.body)
...
const putProvider = async (req,res) => {
    try{
         const providerActualizado = await providersService.updateProviderService(req.params.companyName, req.body);
...
//http://localhost:3000/api/providers/id
 const deleteProvider = async (req, res) => {
    try {
        const { proveedorEliminado, productosDesvinculados } = await providersService.deleteProviderService(req.params.id);
        res.status(200).json({
